...
### Fixed
- **Neo-Geo audio desync** — sound drifted several seconds behind the image on longer sessions. The Neo-Geo audio worker was writing a frame to the SharedArrayBuffer ring buffer every tick without checking whether the AudioWorklet could drain it, so a brief consumer stall would pile samples up and the backlog never recovered. Added the CPS-1 worker's `ringBuffer.freeSlots < 1024` guard — both in the catch-up loop and at the top of `runOneFrame` — so the worker simply skips frames when the consumer is saturated instead of accumulating them.
