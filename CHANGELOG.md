...
### Fixed
- **Neo-Geo palette cache not invalidated on mid-frame writes** — raster effects driven by IRQ2 (the Neo-Geo eye-catcher logo flash, Metal Slug 2's rippling horizon, AoF tilted backgrounds, etc.) were rendered with the palette frozen to its VBlank-time state. The bus now invokes a `paletteWrite` callback on every byte write into `0x400000-0x401FFF`; the callback marks `NeoGeoVideo.paletteDirty`, so the next `renderSlice()` rebuilds the decoded ABGR cache before drawing the slice.
